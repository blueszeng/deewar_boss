...
router.post('/add',function(req,res){
      ...
      var sql = `insert into t_property_value(propertyId, value)
      value(?,?)
      `
      db.query(sql,[propertyData.propertyId, propertyData.value],function(err) {
          if (err)
          {
            console.log(err)
...
router.get('/del.html',function(req,res) {
  ...
  if (!propertyValueId) {
      return res.redirect(`/store/propertyValue/?error=${encodeURI('ID不能为空')}`);
  }
  var sql = `
  ...
  db.query(sql, [propertyValueId], function(err,result) {
      console.log(err)
      if (err){
        return res.redirect(`/store/propertyValue/?error=${encodeURI('无法删除属性')}`);
      }
      return res.redirect(`/store/propertyValue/?info=${encodeURI('删除属性成功')}`);
  });
});
